...
void main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    float ringAmount = ring(pr, uBaseRadius + fi * uRadiusStep, pow(uRingGap, fi),
                            i == 0 ? 0.0 : 2.95 * fi, px);
...
export interface MagicRingsProps {
  ...
  /**
   * When true, mouse events are tracked on the window/document rather than
   * the canvas element. Useful when the canvas sits behind other content
   * (e.g. as a fixed full-page background with pointer-events: none).
   */
  trackGlobal?: boolean;
}
...
export default function MagicRings({
  ...
  // Store latest props in a ref so the animation loop always uses fresh values
  // without needing to re-run the effect.
  const propsRef = useRef({
    color, colorTwo, speed, ringCount, attenuation, lineThickness,
    baseRadius, radiusStep, scaleRate, opacity, noiseAmount,
    rotation, ringGap, fadeIn, fadeOut, followMouse, mouseInfluence,
    hoverScale, parallax, clickBurst, alphaMode, trackGlobal,
  });
  propsRef.current = {
    color, colorTwo, speed, ringCount, attenuation, lineThickness,
    baseRadius, radiusStep, scaleRate, opacity, noiseAmount,
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (propsRef.current.trackGlobal) {
        // Normalize relative to viewport
    ...
    // For global tracking, listen on document so we capture events even when
    // pointer-events: none is set on the container.
    const moveTarget: EventTarget = trackGlobal ? document : mount;
    const enterLeaveTarget: EventTarget = mount;

    moveTarget.addEventListener('mousemove', onMouseMove as EventListener);
    enterLeaveTarget.addEventListener('mouseenter', onMouseEnter);
    enterLeaveTarget.addEventListener('mouseleave', onMouseLeave);

    // For click burst, always listen on document so it works as a global bg
    document.addEventListener('click', onClick);

    // When global tracking, rings are always in "hover" state (full scale)
    if (trackGlobal) isHoveredRef.current = true;
    ...
    const animate = (t: number) => {
      ...
      hoverAmountRef.current += ((isHoveredRef.current ? 1 : 0) - hoverAmountRef.current) * 0.06;
    ...
    const tryStop = () => {
      if (frameId !== 0) { cancelAnimationFrame(frameId); frameId = 0; }
    };
    ...
    return () => {
      ...
      enterLeaveTarget.removeEventListener('mouseenter', onMouseEnter);
      enterLeaveTarget.removeEventListener('mouseleave', onMouseLeave);
